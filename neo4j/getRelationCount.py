...
# 统计专业类型标签下的图书数量
def getBookCounts(graphConn, tags):
    count = 0
    for tag in tags:
        cypher = "MATCH (n:`专业类型标签`{name:'" + tag + "'})-[]-()  RETURN count(*)"
        data = graphConn.run(cypher).to_data_frame()
        count+=data["count(*)"][0]
    return count

def getUserDefinedLabelsCounts(graphConn, title):
    realTitle = "useless"+title
    cypher = "MATCH (n:`无意义标签`{name:'"+realTitle+"'})-[]-() RETURN count(*)"
    data = graphConn.run(cypher).to_data_frame()
    count = 0

    count += data["count(*)"][0]
    print(title,"下的用户自定义标签数量为",count)
if __name__ == '__main__':
    graphConn = getNeo4jConn()
    # # 测试
    # title = "最受欢迎的250本小说"
    # tags = ["Top250"]
    # # 科技
    ...
    tags = ["小说", "文学", "外国文学", "经典", "中国文学", "随笔", "日本文学", "散文",
            ...
            "米兰·昆德拉", "杜拉斯", "港台"]
    count =getBookCounts(graphConn, tags)
    print(title,"下的图书数量为：",count)
    getUserDefinedLabelsCounts(graphConn,title)
